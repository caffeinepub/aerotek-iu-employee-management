...
import { useGetAllEmployees, useGetShiftsForDepartment, useAssignShift, useDeleteShift, usePublishSchedule, useUnpublishSchedule } from '../../hooks/useQueries';
import { Employee, Shift, ShiftStatus } from '../../backend';
...
import { ChevronLeft, ChevronRight, Plus, Trash2, Globe, EyeOff, Moon } from 'lucide-react';
...
export default function SchedulingPage() {
  ...
  const { data: shifts = [], isLoading: shiftsLoading } = useGetShiftsForDepartment(selectedDept);
  const assignShift = useAssignShift();
  const deleteShift = useDeleteShift();
  const publishSchedule = usePublishSchedule();
  const unpublishSchedule = useUnpublishSchedule();

  const departments = Array.from(new Set(employees.map((e: Employee) => e.department).filter(Boolean)));
  ...
  const hasPublished = weekShifts.some((s: Shift) => s.status === ShiftStatus.published);
  const hasDraft = weekShifts.some((s: Shift) => s.status === ShiftStatus.draft);
  ...
  const handleAddShift = async () => {
    ...
    try {
      ...
      toast.error(`Failed to add shift: ${e instanceof Error ? e.message : 'Unknown error'}`);
    }
  };
  ...
  const handleUnpublish = async () => {
    ...
    try {
      ...
      toast.error(`Failed to unpublish: ${e instanceof Error ? e.message : 'Unknown error'}`);
    }
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage and publish employee schedules</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {hasPublished && (
            ...
            >
              ...
              {unpublishSchedule.isPending ? 'Unpublishing...' : 'Unpublish Schedule'}
            </Button>
          )}
          {hasDraft && (
            <Button
      ...
      {!selectedDept ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-7 gap-3">
          {DAYS.map((d) => <Skeleton key={d} className="h-32 w-full" />)}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-7 gap-3">
          {DAYS.map((day, idx) => {
            ...
            return (
              <Card key={day} className={`min-h-[140px] ${isToday ? 'border-primary' : ''}`}>
                <CardHeader className="pb-2 pt-3 px-3">
                  <CardTitle className="text-xs font-semibold flex items-center justify-between">
                    <span className={isToday ? 'text-primary' : ''}>{day}</span>
                    <span className={`text-xs font-normal ${isToday ? 'text-primary' : 'text-muted-foreground'}`}>
                      {dayDate.getDate()}
                    </span>
                  </CardTitle>
                </CardHeader>
                <CardContent className="px-3 pb-3 space-y-1">
                  {dayShifts.map((shift: Shift) => {
                    const emp = employees.find((e: Employee) => e.id === shift.employeeId);
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge
                          variant={shift.status === ShiftStatus.published ? 'default' : 'secondary'}
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => setNewShift((s) => ({ ...s, employeeId: v }))}
              >
                <SelectTrigger>
