...
import { UserCog, Plus, Eye, EyeOff, AlertCircle, Info, Users } from 'lucide-react';
import { toast } from 'sonner';
import { Role } from '../../backend';
import { useCreateAccount, extractErrorMessage } from '../../hooks/useQueries';

const ROLE_OPTIONS = [
  { value: Role.hrAdmin, label: 'HR Admin', color: 'text-purple-400', badge: 'bg-purple-500/15 border-purple-500/20' },
  { value: Role.manager, label: 'Manager', color: 'text-blue-400', badge: 'bg-blue-500/15 border-blue-500/20' },
  { value: Role.supervisor, label: 'Supervisor', color: 'text-teal-400', badge: 'bg-teal-500/15 border-teal-500/20' },
  { value: Role.employee, label: 'Employee', color: 'text-green-400', badge: 'bg-green-500/15 border-green-500/20' },
];
...
export default function AccountManagementPage() {
  ...
  const resetForm = () => {
    setForm({ displayName: '', username: '', password: '', confirmPassword: '', role: Role.employee });
    setErrors({});
    setServerError('');
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-navy-400 text-sm">Create and manage user login accounts</p>
        </div>
        <button
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-blue-300 text-sm font-medium">Pre-seeded Admin Accounts</p>
          <p className="text-blue-400/70 text-xs mt-0.5">
            <strong>richaadi0</strong> and <strong>Adirich</strong> are pre-configured HR Admin accounts (password: 3vtpxdgf). These accounts are always available.
          </p>
        </div>
      </div>
      ...
      <div className="bg-navy-900/60 border border-navy-700/50 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { role: 'HR Admin', color: 'purple', perms: ['Full system access', 'Manage all roles', 'Hire & terminate', 'View all data'] },
            { role: 'Manager', color: 'blue', perms: ['Manage team', 'Schedule shifts', 'Approve time off', 'Create Employee/Manager accounts'] },
            { role: 'Supervisor', color: 'teal', perms: ['View all schedules', 'Approve time off', 'View team data', 'Read-only access'] },
            { role: 'Employee', color: 'green', perms: ['View own schedule', 'Submit time off', 'View own data', 'Self-service portal'] },
          ].map(({ role, color, perms }) => (
            <div key={role} className={`bg-${color}-500/5 border border-${color}-500/15 rounded-xl p-4`}>
              <p className={`text-${color}-400 font-semibold text-sm mb-2`}>{role}</p>
              <ul className="space-y-1">
                {perms.map(p => (
                  <li key={p} className="text-navy-400 text-xs flex items-center gap-1.5">
                    <span className={`w-1 h-1 rounded-full bg-${color}-500/60`} />
                    {p}
                  </li>
                ))}
              </ul>
          ...
          <div className="bg-navy-900 border border-navy-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                />
                {errors.displayName && <p className="text-red-400 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{errors.displayName}</p>}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-navy-400 hover:text-navy-200">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-navy-400 hover:text-navy-200">
              ...
              <div>
                <label className="block text-sm font-medium text-navy-200 mb-1.5">Role *</label>
                <select
                  value={form.role}
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {createMutation.isPending ? (
                    <><div className="w-3.5 h-3.5 border-2 border-navy-950/30 border-t-navy-950 rounded-full animate-spin" />Creating...</>
                  ) : 'Create Account'}
                </button>
              </div>
            </form>
