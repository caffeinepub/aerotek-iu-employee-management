...
import { Clock, CheckCircle, XCircle, MessageSquare, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function SupervisorTimeOffPage() {
  ...
  const approveMutation = useMutation({
    mutationFn: async ({ id, comment }: { id: string; comment: string }) => {
      ...
      return actor.approveTimeOffRequestWithComment(id, comment || 'Approved by supervisor');
    },
    onSuccess: () => {
  ...
  const statusBadge = (status: TimeOffStatus) => {
    if (status === TimeOffStatus.approved) {
      return <span className="px-2.5 py-1 bg-green-500/15 border border-green-500/20 text-green-400 text-xs rounded-full font-medium">Approved</span>;
    }
    if (status === TimeOffStatus.denied) {
      return <span className="px-2.5 py-1 bg-red-500/15 border border-red-500/20 text-red-400 text-xs rounded-full font-medium">Denied</span>;
    }
    return <span className="px-2.5 py-1 bg-amber-500/15 border border-amber-500/20 text-amber-400 text-xs rounded-full font-medium">Pending</span>;
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-navy-400 text-sm ml-7">Review and action employee time off requests</p>
      </div>
      ...
      <div className="bg-navy-900/60 border border-navy-700/50 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-navy-700/30 flex items-center justify-between">
          <h2 className="text-white font-semibold flex items-center gap-2">
            ...
            Pending Requests
            {pending.length > 0 && (
              <span className="ml-1 px-2 py-0.5 bg-amber-500/20 text-amber-400 text-xs rounded-full">{pending.length}</span>
            )}
          </h2>
        </div>
        ...
        {isLoading ? (
          <div className="p-6 space-y-3">
            {[1, 2, 3].map(i => <div key={i} className="h-16 bg-navy-800/50 rounded-xl animate-pulse" />)}
          </div>
        ) : pending.length === 0 ? (
            ...
            {pending.map(req => (
              <div key={req.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-white font-medium text-sm">{getEmployeeName(req.employeeId)}</p>
                    <span className="text-navy-500 text-xs">·</span>
                    <span className="text-navy-400 text-xs">{req.timeOffType}</span>
                  </div>
                  <p className="text-navy-400 text-xs">
                    {new Date(Number(req.startDate)).toLocaleDateString()} – {new Date(Number(req.endDate)).toLocaleDateString()}
                  </p>
                  <p className="text-navy-500 text-xs mt-0.5">
                    Requested: {new Date(Number(req.requestDate)).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => approveMutation.mutate({ id: req.id, comment: 'Approved by supervisor' })}
      ...
      <div className="bg-navy-900/60 border border-navy-700/50 rounded-2xl overflow-hidden">
        ...
        >
          <h2 className="text-white font-semibold flex items-center gap-2">
            ...
            Processed Requests
            {processed.length > 0 && (
              <span className="ml-1 px-2 py-0.5 bg-navy-700 text-navy-300 text-xs rounded-full">{processed.length}</span>
            )}
          </h2>
          {showProcessed ? <ChevronUp className="w-4 h-4 text-navy-400" /> : <ChevronDown className="w-4 h-4 text-navy-400" />}
        </button>
                ...
                <div key={req.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex-1">
                    ...
                    {req.approverComments && (
                      <p className="text-navy-500 text-xs mt-1 italic">"{req.approverComments}"</p>
                    )}
                  </div>
      ...
      {denyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-navy-900 border border-navy-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <p className="text-navy-400 text-sm mb-4">Please provide a reason for denying this time off request.</p>
            <textarea
              value={denyComment}
              onChange={e => { setDenyComment(e.target.value); setDenyCommentError(''); }}
              placeholder="Enter reason for denial..."
              rows={3}
              className="w-full px-4 py-3 bg-navy-800/60 border border-navy-600/50 rounded-xl text-white placeholder-navy-400 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50 resize-none"
            />
            {denyCommentError && (
            ...
            <div className="flex gap-3 mt-4">
              <button
                onClick={() => { setDenyModalOpen(false); setDenyRequestId(null); }}
              ...
              <button
                ...
                disabled={denyMutation.isPending}
                className="flex-1 px-4 py-2.5 bg-red-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {denyMutation.isPending ? (
                  <><div className="w-3.5 h-3.5 border-2 border-red-400/30 border-t-red-400 rounded-full animate-spin" /> Denying...</>
                ) : 'Confirm Deny'}
              </button>
            </div>
          </div>
