...
import { Users, Calendar, Clock, ArrowRight, TrendingUp, CheckCircle } from 'lucide-react';
...
export default function SupervisorDashboard() {
  ...
  const { data: timeOffRequests = [], isLoading: torLoading } = useTimeOffRequests();
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-navy-400 text-sm ml-9">Overview of your team's activity and requests</p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${c.badge}`}>{kpi.sub}</span>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-navy-400 text-xs">Browse weekly shift calendar</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-white font-semibold text-sm">Time Off Requests</p>
              <p className="text-navy-400 text-xs">
                {pendingRequests.length > 0 ? `${pendingRequests.length} pending review` : 'No pending requests'}
              </p>
            </div>
          </div>
      ...
      <div className="bg-navy-900/60 border border-navy-700/50 rounded-2xl p-6">
        ...
        {torLoading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-12 bg-navy-800/50 rounded-xl animate-pulse" />
            ))}
          </div>
        ) : pendingRequests.length === 0 ? (
          <div className="text-center py-8">
            <CheckCircle className="w-10 h-10 text-teal-500/40 mx-auto mb-2" />
            <p className="text-navy-400 text-sm">All caught up! No pending requests.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {pendingRequests.slice(0, 5).map(req => (
              <div key={req.id} className="flex items-center justify-between p-3 bg-navy-800/40 rounded-xl border border-navy-700/30">
                <div>
                  <p className="text-white text-sm font-medium">{req.employeeId}</p>
                  <p className="text-navy-400 text-xs">{req.timeOffType} · {new Date(Number(req.startDate)).toLocaleDateString()}</p>
                </div>
