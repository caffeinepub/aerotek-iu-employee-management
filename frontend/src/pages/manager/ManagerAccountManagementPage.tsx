...
import { UserCog, Plus, Eye, EyeOff, AlertCircle, Info } from 'lucide-react';
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-navy-400 text-sm">Create Employee and Manager login accounts</p>
        </div>
        <button
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-blue-300 text-sm font-medium">Manager Account Permissions</p>
          <p className="text-blue-400/70 text-xs mt-0.5">
            As a Manager, you can create <strong>Employee</strong> and <strong>Manager</strong> accounts only. HR Admin and Supervisor roles can only be assigned by HR Admins.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-green-500/5 border border-green-500/15 rounded-2xl p-5">
          ...
          <ul className="space-y-1.5">
            {['View own schedule', 'Submit time off requests', 'View own PTO balance', 'Self-service portal'].map(p => (
              <li key={p} className="text-navy-400 text-xs flex items-center gap-1.5">
                <span className="w-1 h-1 rounded-full bg-green-500/60" />{p}
              </li>
            ))}
          </ul>
        </div>
        <div className="bg-blue-500/5 border border-blue-500/15 rounded-2xl p-5">
          ...
          <ul className="space-y-1.5">
            {['Manage team members', 'Schedule shifts', 'Approve time off', 'Create Employee/Manager accounts'].map(p => (
              <li key={p} className="text-navy-400 text-xs flex items-center gap-1.5">
                <span className="w-1 h-1 rounded-full bg-blue-500/60" />{p}
              </li>
            ))}
          </ul>
        </div>
