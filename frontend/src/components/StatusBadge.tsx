...
type StatusType =
  ...
  | 'open' | 'closed' | 'draft'
  | 'hired' | 'rejected' | 'applied' | 'screening' | 'interview' | 'offer'
...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  hrAdmin: { label: 'HR Admin', className: 'bg-purple-100 text-purple-800 border border-purple-200' },
  manager: { label: 'Manager', className: 'bg-blue-100 text-blue-800 border border-blue-200' },
  employee: { label: 'Employee', className: 'bg-gray-100 text-gray-700 border border-gray-200' },
};

export default function StatusBadge({ status, className = '' }: StatusBadgeProps) {
  const config = statusConfig[status] || { label: status, className: 'bg-gray-100 text-gray-700 border border-gray-200' };
  return (
