...
const navItems = [
  ...
  { to: "/supervisor/schedule", label: "Schedule", icon: Calendar },
  { to: "/supervisor/time-off", label: "Time Off Requests", icon: Clock },
  { to: "/supervisor/timesheets", label: "Timesheets", icon: ClipboardList },
];
...
export default function SupervisorLayout() {
  ...
  // Enable real-time toast popups for new notifications
  ...
  const isActive = (to: string) => {
    if (to === "/supervisor")
      return currentPath === "/supervisor" || currentPath === "/supervisor/";
  ...
  // Show displayName if available, fallback to username
  ...
  return (
    <div className="flex h-screen bg-[oklch(0.97_0.005_240)] overflow-hidden">
      {/* Backdrop overlay */}
      {drawerOpen && (
        // biome-ignore lint/a11y/useKeyWithClickEvents: backdrop click-to-close is supplementary
        <div
      ...
      >
            ...
            <div className="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 bg-[oklch(0.78_0.16_75)] flex items-center justify-center">
              <img
                ...
                onError={(e) => {
                  ...
                  if (p)
                    p.innerHTML =
                      '<span style="font-weight:800;color:oklch(0.15 0.02 255);font-size:1.1rem">A</span>';
                }}
              />
            </div>
            <div>
              ...
              <p className="text-[oklch(0.78_0.16_75)] text-xs">
                (IU) Employee Mgmt
              </p>
            </div>
          </div>
        ...
        <div className="px-5 py-3 border-b border-[oklch(0.28_0.04_255)] flex-shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-[oklch(0.65_0.18_30)] flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-[oklch(0.78_0.16_75)] text-xs">Supervisor</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-[oklch(0.88_0.01_240)] px-4 py-3 flex items-center justify-between flex-shrink-0 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm text-[oklch(0.52_0.02_250)]">
              ...
              <span className="hidden sm:inline">
                Aerotek (IU) Employee Management
              </span>
              <ChevronRight className="w-3 h-3 hidden sm:inline" />
              <span className="text-[oklch(0.18_0.04_255)] font-medium">
                Supervisor Portal
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {/* Notification Bell */}
            <NotificationBell />
            <span className="text-sm text-[oklch(0.35_0.02_250)] hidden sm:block font-medium">
              {displayName}
            </span>
            <span className="text-xs bg-[oklch(0.65_0.18_30)] text-white px-2.5 py-1 rounded-full font-semibold">
              Supervisor
            </span>
          </div>
        </header>

        {/* Snow / Overtime Warning Banner */}
        <div className="bg-red-700 text-white px-6 py-3 flex items-start gap-3 flex-shrink-0">
          <AlertTriangle className="w-5 h-5 flex-shrink-0 mt-0.5" />
          <p className="text-sm font-bold leading-snug">
            ⚠️ All staff who work at Owen, Wily, Kirkwood please park in parking
            lot not right by the building thank you if you have any questions
            contact Kip or Jai or your immediate supervisor.
          </p>
        </div>
