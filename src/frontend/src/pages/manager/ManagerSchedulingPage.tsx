...
import { type Shift, ShiftStatus } from "../../backend";
...
export default function ManagerSchedulingPage() {
  ...
  // Determine if the current week's schedule is published
  ...
  const _hasDraftShifts = useMemo(() => {
    return weekShifts.some((s) => s.status === ShiftStatus.draft);
  }, [weekShifts]);
  ...
  async function handleAddShift() {
    ...
    try {
      ...
      toast.error(
        `Failed to add shift: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async function handlePublishToggle() {
    ...
    try {
      ...
      toast.error(
        `Failed to update schedule: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      {weekShifts.length > 0 && (
        ...
        >
          {isPublished ? (
            <>
              <Send className="h-4 w-4" /> This week's schedule is published and
              visible to employees.
            </>
          ) : (
            <>
              <EyeOff className="h-4 w-4" /> This week's schedule has{" "}
              {weekShifts.filter((s) => s.status === ShiftStatus.draft).length}{" "}
              draft shift(s) — not yet visible to employees.
            </>
          )}
        </div>
      )}
                        ...
                        <div className="flex items-center justify-between gap-1">
                          ...
                          <div className="flex items-center gap-0.5 shrink-0">
                            ...
                            {isDraft && (
                              <Badge
                                variant="outline"
                                className="text-[9px] px-1 py-0 h-4 border-amber-400 text-amber-700 dark:text-amber-300"
                              >
                                Draft
                              </Badge>
                            )}
