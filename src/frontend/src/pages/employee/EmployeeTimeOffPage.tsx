...
import {
  useGetTimeOffRequestsForEmployee,
  useSubmitTimeOffRequest,
} from "../../hooks/useQueries";
// Inline helpers (lib/utils only exports cn)
...
export default function EmployeeTimeOffPage() {
  ...
  const { data: requests, isLoading } =
    useGetTimeOffRequestsForEmployee(employeeId);
  ...
  const approved =
    requests?.filter((r) => r.status === TimeOffStatus.approved) ?? [];
  const denied =
    requests?.filter((r) => r.status === TimeOffStatus.denied) ?? [];
    ...
    <div className="p-6 space-y-6">
      ...
      {!employeeId && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
          Your account is not linked to an employee profile. Please contact HR
          to link your account before submitting time off requests.
        </div>
      )}
      ...
      <Card className="border border-[oklch(0.88_0.01_240)] shadow-card">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="space-y-2">
              {[...requests]
                .sort((a, b) => Number(b.requestDate) - Number(a.requestDate))
                .map((req) => (
                  <div
                    key={req.id}
                    className="flex items-center justify-between py-2.5 border-b border-[oklch(0.94_0.008_240)] last:border-0"
                  >
                    <div>
                      ...
                      {req.approverComments && (
                        <p className="text-xs text-[oklch(0.52_0.02_250)] italic mt-0.5">
                          "{req.approverComments}"
                        </p>
                      )}
                    </div>
