    ...
    <tr className="border-b border-[oklch(0.94_0.008_240)] last:border-0 hover:bg-[oklch(0.97_0.005_240)]">
      <td className="py-3 px-4">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-medium text-sm text-[oklch(0.18_0.04_255)]">
            {employee.fullName}
          </span>
        </div>
      </td>
      ...
      <td className="py-3 px-4">
        {isLoading ? (
          ...
          <span className="text-lg font-bold text-[oklch(0.22_0.04_255)]">
...
export default function PTOBalancesPage() {
  ...
  const handleAdjust = async () => {
    ...
    if (!adjustForm.balance || Number.isNaN(Number(adjustForm.balance))) {
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center gap-3 mb-2">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate({ to: "/hr/pto-policies" })}
        >
        ...
        <div>
          ...
          <p className="text-sm text-[oklch(0.52_0.02_250)]">
            View and adjust employee PTO balances
          </p>
        </div>
      </div>
        ...
        {empLoading ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-[oklch(0.97_0.005_240)] border-b border-[oklch(0.88_0.01_240)]">
              <tr>
                <th className="text-left py-3 px-4 text-xs font-semibold text-[oklch(0.52_0.02_250)] uppercase tracking-wide">
                  Employee
                </th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-[oklch(0.52_0.02_250)] uppercase tracking-wide">
                  Department
      ...
      <Dialog
        open={!!adjustModal}
        onOpenChange={(open) => {
          if (!open) setAdjustModal(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Adjust PTO Balance — {adjustModal?.fullName}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Select
                value={adjustForm.policyId}
                onValueChange={(v) =>
                  setAdjustForm((f) => ({ ...f, policyId: v }))
                }
              >
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setAdjustForm((f) => ({ ...f, balance: e.target.value }))
                }
