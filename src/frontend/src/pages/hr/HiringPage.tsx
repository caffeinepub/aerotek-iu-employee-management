...
export default function HiringPage() {
  ...
  const grouped = {
    ...
    [JobPostingStatus.draft]:
      postings?.filter((p) => p.status === JobPostingStatus.draft) ?? [],
    [JobPostingStatus.closed]:
      postings?.filter((p) => p.status === JobPostingStatus.closed) ?? [],
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[oklch(0.52_0.02_250)] mt-0.5">
            {postings?.length ?? 0} total job postings
          </p>
        </div>
        <Button
          onClick={() => navigate({ to: "/hr/hiring/new" })}
          ...
          {columns.map((col) => (
            <div key={col.status}>
              <div className="flex items-center gap-2 mb-3">
                <h2 className="font-semibold text-[oklch(0.18_0.04_255)]">
                  {col.label}
                </h2>
                ...
                {grouped[col.status].length === 0 ? (
                  ...
                  grouped[col.status].map((posting) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate({
                          to: "/hr/hiring/$postingId/pipeline",
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="font-semibold text-[oklch(0.18_0.04_255)] text-sm leading-tight">
                            {posting.title}
                          </h3>
                          <StatusBadge status={posting.status} />
                        </div>
                        <p className="text-xs text-[oklch(0.52_0.02_250)] mb-3">
                          {posting.department}
                        </p>
                        <p className="text-xs text-[oklch(0.52_0.02_250)] line-clamp-2 mb-3">
                          {posting.description}
                        </p>
                        <div className="flex items-center justify-between">
