...
export default function PTOPolicyFormPage() {
  ...
  const addLeaveType = () => {
    ...
    if (t && !form.eligibleLeaveTypes.includes(t)) {
      setForm((f) => ({
        ...f,
        eligibleLeaveTypes: [...f.eligibleLeaveTypes, t],
        leaveTypeInput: "",
      }));
    }
  };
  ...
  const removeLeaveType = (type: string) => {
    setForm((f) => ({
      ...f,
      eligibleLeaveTypes: f.eligibleLeaveTypes.filter((t) => t !== type),
    }));
  };
  ...
  const validate = () => {
    ...
    if (!form.accrualRate || Number.isNaN(Number(form.accrualRate)))
      e.accrualRate = "Valid accrual rate required";
    if (!form.maxCarryOver || Number.isNaN(Number(form.maxCarryOver)))
      e.maxCarryOver = "Valid max carry-over required";
    setErrors(e);
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[oklch(0.18_0.04_255)]">
            {isEdit ? "Edit" : "Create"} PTO Policy
          </h1>
          <p className="text-sm text-[oklch(0.52_0.02_250)]">
            {isEdit ? "Update policy details" : "Define a new PTO policy"}
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, accrualRate: e.target.value }))
                  }
                  placeholder="1.5"
                  className="mt-1 h-9"
                />
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, leaveTypeInput: e.target.value }))
                  }
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
