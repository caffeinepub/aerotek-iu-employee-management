...
const STAGES = [
  ...
  {
    key: ApplicantStage.screening,
    label: "Screening",
    color: "border-t-indigo-400",
  },
  {
    key: ApplicantStage.interview,
    label: "Interview",
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 overflow-x-auto">
          {STAGES.map((stage) => {
            const stageApplicants =
              applicants?.filter((a) => a.stage === stage.key) ?? [];
            return (
              <div key={stage.key} className="min-w-36">
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="text-xs font-semibold text-[oklch(0.35_0.02_250)] uppercase tracking-wide">
                    {stage.label}
                  </span>
                  <span className="text-xs bg-[oklch(0.94_0.008_240)] text-[oklch(0.52_0.02_250)] px-1.5 py-0.5 rounded-full">
                ...
                <div className="space-y-2">
                  {stageApplicants.length === 0 ? (
                    ...
                    stageApplicants.map((applicant) => (
                      <Card
                        key={applicant.id}
                        className={`border border-[oklch(0.88_0.01_240)] shadow-xs border-t-4 ${stage.color}`}
                      >
                        <CardContent className="p-3">
                          <p className="font-medium text-xs text-[oklch(0.18_0.04_255)] mb-1">
                            {applicant.name}
                          </p>
                          <p className="text-xs text-[oklch(0.52_0.02_250)] mb-2">
                            {applicant.contact}
                          </p>
                          <p className="text-xs text-[oklch(0.65_0.02_250)] mb-2 line-clamp-2">
                            {applicant.resumeNotes}
                          </p>
                          <Select
                            value={applicant.stage}
                            onValueChange={(v) =>
                              handleMoveStage(applicant.id, v as ApplicantStage)
                            }
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewApplicant((a) => ({ ...a, name: e.target.value }))
                }
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewApplicant((a) => ({ ...a, contact: e.target.value }))
                }
            ...
            <div>
              ...
              <Textarea
                value={newApplicant.resumeNotes}
                onChange={(e) =>
                  setNewApplicant((a) => ({
                    ...a,
                    resumeNotes: e.target.value,
                  }))
                }
                placeholder="Key qualifications, experience..."
                className="mt-1"
              />
            </div>
          </div>
